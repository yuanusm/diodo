...
Uso:
  ...
  python ajuste_diodo_riguroso.py --csv datos.csv --current-unit mA --plot ajuste.png
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ajuste riguroso de diodos con varios modelos físicos.")
    parser.add_argument("--csv", default="datos.csv", type=Path, help="Archivo CSV de entrada.")
    parser.add_argument("--voltage-col", default="Voltaje", help="Nombre de la columna de voltaje.")
    parser.add_argument("--current-col", default="Corriente", help="Nombre de la columna de corriente.")
    parser.add_argument("--current-unit", default="mA", choices=["A", "mA", "uA", "µA", "nA"], help="Unidad de la columna de corriente.")
    parser.add_argument("--plot", default=Path("ajuste_diodo_riguroso.png"), type=Path, help="Ruta del gráfico de salida.")
    parser.add_argument("--no-plot", action="store_true", help="No generar gráfico; útil en entornos sin matplotlib.")
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    v, i = load_data(args.csv, args.voltage_col, args.current_col, args.current_unit)
    if len(v) < 6:
    ...
    if args.no_plot:
        print("Gráfico omitido por --no-plot")
    else:
        make_plot(v, i, results, args.plot)
        print(f"Gráfico guardado en: {args.plot}")
